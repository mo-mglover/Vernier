...
// Make sure the code exits wnen an unknown hash is passed to Vernier. i.e. A
// hash which was not generated by a start calliper.
TEST(DeathTest, UnknownHashTest) {
  ...
  EXPECT_EXIT(
      {
        ...
        // Create a previously-unused hash, and pass to stop calliper.
        const auto unknown_hash = std::hash<std::string_view>{}("Indigestible");
        meto::vernier.stop(unknown_hash);

        // End of main region
        meto::vernier.stop(prof_main);
      },
      ExitedWithCode(EXIT_FAILURE), "EMERGENCY STOP: received unknown hash.");
